...
def SearchBar(request):
    if request.method == 'POST':
        ...
        search_result = Product.objects.filter(product_name__contains=searched)
        return render(request, 'search/searchbar.html', {'searched':search_result, 'term':searched})
...
def SearchByOEM(request):
    if request.method=="POST":
        product_OEM = request.POST.get('OEM')
        ...
        return render(request, 'search/OEM.html', {'product':filter_oem, 'term':request.POST.get('OEM')})